...
const getPostTimeline = `-- name: GetPostTimeline :many
SELECT
...
type GetPostTimelineRow struct {
	Post           Post  `json:"post"`
	User           User  `json:"user"`
...
func (q *Queries) GetPostTimeline(ctx context.Context, arg GetPostTimelineParams) ([]GetPostTimelineRow, error) {
	rows, err := q.db.Query(ctx, getPostTimeline, arg.Limit, arg.SelfID, arg.CreatedAt)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []GetPostTimelineRow{}
	for rows.Next() {
		var i GetPostTimelineRow
		if err := rows.Scan(
			&i.Post.ID,
			&i.Post.AuthorID,
			&i.Post.ReplyToID,
