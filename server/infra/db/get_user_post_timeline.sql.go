...
// versions:
//   sqlc v1.27.0
// source: get_user_post_timeline.sql
...
const getUserPostTimeline = `-- name: GetUserPostTimeline :many
SELECT
...
type GetUserPostTimelineRow struct {
	Post           Post  `json:"post"`
	User           User  `json:"user"`
...
func (q *Queries) GetUserPostTimeline(ctx context.Context, arg GetUserPostTimelineParams) ([]GetUserPostTimelineRow, error) {
	rows, err := q.db.Query(ctx, getUserPostTimeline,
		arg.Limit,
		arg.SelfID,
		arg.AuthorID,
		arg.CreatedAt,
	...
	defer rows.Close()
	items := []GetUserPostTimelineRow{}
	for rows.Next() {
		var i GetUserPostTimelineRow
		if err := rows.Scan(
			&i.Post.ID,
			&i.Post.AuthorID,
			&i.Post.ReplyToID,
